...
/**
 * Converts a single bote-annotated Smithy service into an AsyncAPI 3.0.0
 * document.
 *
 ...
 *
 * <ul>
 *   <li>the Kafka protocol service     -&gt; the AsyncAPI document</li>
 *   <li>{@code @kafkaTopic}            -&gt; a channel (with Kafka channel binding)</li>
 *   <li>{@code @send}/{@code @receive} -&gt; an operation with action send/receive</li>
 *   <li>each message value structure   -&gt; a component message + JSON Schema payload</li>
 *   <li>{@code @kafkaKey}              -&gt; the Kafka message binding key</li>
 *   <li>{@code @kafkaHeader}           -&gt; the message headers schema</li>
 *   <li>{@code @kafkaTopicConfig}      -&gt; Kafka channel binding partitions/replicas/config</li>
 * </ul>
 */
final class AsyncApiConverter {
    ...
    /**
     * A {@code @receive} operation's output contains a member targeting a
     * {@code @streaming} union; each union member is a possible message type.
     */
    ...
    private Node operationNode(OperationShape operation, String action, String topic, Set<ShapeId> messages) {
    ...
    private Node buildChannels() {
        ...
        for (Channel channel : channels.values()) {
            ...
            for (ShapeId messageId : channel.messages) {
                messages.withMember(messageId.getName(), ref("#/components/messages/" + messageId.getName()));
            }
            builder.withMember(channel.topic, Node.objectNodeBuilder()
    ...
    private Node channelBindings(Channel channel) {
        ObjectNode.Builder kafka = Node.objectNodeBuilder()
    ...
    private Node buildMessage(ShapeId messageId) {
        ...
        ObjectNode.Builder message = Node.objectNodeBuilder()
        ...
        keyMember(structure).ifPresent(key -> message.withMember("bindings", Node.objectNodeBuilder()
                .withMember("kafka", Node.objectNodeBuilder()
    ...
    private Node buildHeaders(StructureShape structure) {
        ...
        for (MemberShape member : structure.getAllMembers().values()) {
            Optional<String> headerName = member.findTrait(KAFKA_HEADER)
    ...
    private Node buildSchemas() {
        ...
        for (ShapeId messageId : messageShapes) {
            model.getShape(messageId).ifPresent(shape ->
                    walker.walkShapes(shape).forEach(s -> closure.add(s.getId())));
        }
    ...
    private String topicName(OperationShape operation) {
        return operation.findTrait(KAFKA_TOPIC)
                .map(t -> t.toNode().expectObjectNode().expectStringMember("name").getValue())
